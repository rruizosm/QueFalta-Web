// ui.jsx — primitivas que se reestilizan según los tokens del tema.
import React from 'react';
import { useTk, Icon, hexA, onAccent } from './theme.jsx';

function pad(t) { return t.sp(t.id === 'mono' ? 22 : 18); }
export const screenPad = pad;
...
export function Scroll({ children, style = {}, padTop = 0 }) {
  const t = useTk();
  return (
...
export function Card({ children, style = {}, flat, accent, onAlt }) {
  const t = useTk();
...
export function Title({ children, size = 26, style = {} }) {
  const t = useTk();
  return (
    <h1 style={{
...
export function Chip({ children, active, style = {}, color }) {
  const t = useTk();
...
export function Btn({ children, icon, variant = 'solid', full, onClick, style = {} }) {
  const t = useTk();
...
export function IconBtn({ icon, onClick, soft, size = 38, style = {} }) {
  const t = useTk();
  return (
    <button onClick={onClick} style={{
...
export function Field({ icon, placeholder, style = {} }) {
  const t = useTk();
  return (
    <div style={{
