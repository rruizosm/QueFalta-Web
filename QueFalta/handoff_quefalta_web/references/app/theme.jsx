// theme.jsx — 4 design languages for LaCompra + live-tweak resolution + icon set.
// Exports to window: THEMES, TWEAK_DEFAULTS, resolveTheme, useTk, ThemeCtx, Icon, hexA, onAccent
...
// Tweak font choices map to a {display, body} pair
const FONT_CHOICES = {
  'Auto':              null,
...
// ── helpers ───────────────────────────────────────────────────
function hexA(hex, a) {
  // hex like #rrggbb + alpha 0..1 → rgba
  const h = hex.replace('#', '');
...
function onAccent(hex) { return lum(hex) > 0.62 ? '#15140f' : '#ffffff'; }

// ── The 4 design languages ────────────────────────────────────
const THEMES = [
  {
...
function useTk() { return React.useContext(ThemeCtx); }
...
function Icon({ name, size = 20, color = 'currentColor', stroke, style = {} }) {
  const tk = useTk();
...
Object.assign(window, { THEMES, TWEAK_DEFAULTS, resolveTheme, useTk, ThemeCtx, Icon, hexA, onAccent, FONT_CHOICES });
